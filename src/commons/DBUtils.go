...
func OpenConnWithTx() (err error) {
	// 此处为等号，否则创建局部变量
	db, err = sql.Open("mysql", "root:admin@tcp(localhost:3306)/ego")
	if err != nil {
		fmt.Println("打开连接的错误", err)
		return
	}
	// 开始事务
	tx,err =db.Begin()
...
func Dql(sql string,args ... interface{}) (*sql.Rows,error)  {
	...
	stmt,err = db.Prepare(sql)
	if err != nil {
		fmt.Println("执行Dql时出现错误，预处理失败")
		return  nil,err
	}
	//此处是切片
	rows ,err = stmt.Query(args...)
	if err != nil {
		fmt.Println("执行Dql时出现错误，执行错误")
		return  nil,err
	}
	// 此处没有关闭，调用此函数要记得关闭连接
	return rows,nil
